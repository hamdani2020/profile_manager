import os
current_user_directory = __import__('login').current_user_directory
get_profile = __import__('login').get_profile
read_profile = __import__('login').read_profile
update_profile = __import__('login').update_profile
...
def main():
    ...
    if os.path.basename(current_dir) == username:
        ...
        if os.path.isfile(profile_path):
            profile_content = read_profile(profile_path)

            confirm = input("This is your updated information? (yes/no): ").strip().lower()
            if confirm != 'yes':
